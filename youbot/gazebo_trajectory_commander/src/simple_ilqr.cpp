// origin of obstacle xyz="${5*0.6} -0.15 0.0" rpy="0 0 0"
// Lekan Ogunmolu
#include "ros/ros.h"
...
void movePlatform() {
	...
	jointvalues[3] = 1.73;
	jointvalues[4] = 2.95;
	ROS_INFO("moving arm straight up");
	msg = createArmPositionCommand(jointvalues);
	armPublisher.publish(msg);
	ros::Duration(5).sleep();

	// open gripper
	gripperMsg = createGripperPositionCommand(0.011);
	ROS_INFO("Opening gripper by 0.011m");
	gripperPublisher.publish(gripperMsg);
	ros::Duration(3).sleep();

	// move forward
	twist.linear.x = 0.1;  
	ROS_INFO("moving forward by 0.1m");
	...
	twist.linear.x = 0;
	twist.linear.y = 0.1;
	ROS_INFO("moving right by 0.1m");
	platformPublisher.publish(twist);
	ros::Duration(3).sleep();

	// back to position diagonally
	twist.linear.x = -0.1;
	twist.linear.y = -0.1;
	ROS_INFO("moving back to position");
	platformPublisher.publish(twist);
	ros::Duration(3).sleep();
	...
	twist.linear.y = 0;
	twist.linear.z = 0;
	ROS_INFO("stopping linear motion commands");
	platformPublisher.publish(twist);
	...
	jointvalues[3] = 0.11;
	jointvalues[4] = 0.111;
	ROS_INFO("moving arm back to rest position");
	msg = createArmPositionCommand(jointvalues);
	armPublisher.publish(msg);
	ros::Duration(5).sleep();

	// close gripper
	gripperMsg = createGripperPositionCommand(0);
	ROS_INFO("Closing gripper by 0.011m");
	gripperPublisher.publish(gripperMsg);
}
